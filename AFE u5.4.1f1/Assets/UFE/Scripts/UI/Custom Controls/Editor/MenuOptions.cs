...
namespace UnityEditor.UI
{
    ...
    static internal class MenuOptions
    {
		...
		[MenuItem("GameObject/UI/UFE Slider", false, 2006)]
		static public void AddMaskSlider(MenuCommand menuCommand)
		{
			// Create GOs Hierarchy
			GameObject root = CreateUIElementRoot("Mask Slider", menuCommand, s_ThinGUIElementSize);
			
			GameObject background = CreateUIObject("Background", root);
			GameObject fillArea = CreateUIObject("Fill Area", root);
			GameObject mask = CreateUIObject("Mask", fillArea);
			GameObject fill = CreateUIObject("Fill", mask);
			GameObject handleArea = CreateUIObject("Handle Slide Area", root);
			GameObject handle = CreateUIObject("Handle", handleArea);
			...
			fillAreaRect.sizeDelta = new Vector2(-2, -4);

			// Mask
			mask.AddComponent<Mask>().showMaskGraphic = false;
			Image maskImage = mask.AddComponent<Image>();
			maskImage.sprite = backgroundImage.sprite;
			maskImage.type = backgroundImage.type;
			...
			// Setup slider component
			/*UFESlider slider = root.AddComponent<UFESlider>();
			slider.maskRect = mask.GetComponent<RectTransform>();
			slider.fillRect = fill.GetComponent<RectTransform>();
			slider.handleRect = handle.GetComponent<RectTransform>();
			slider.targetGraphic = handleImage;
			slider.direction = UFESlider.Direction.LeftToRight;
			SetDefaultColorTransitionValues(slider);*/
		}
