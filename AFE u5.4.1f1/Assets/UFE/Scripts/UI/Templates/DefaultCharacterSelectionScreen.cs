			...
			if (characterIndex >= 0 && characterIndex <= maxCharacterIndex){
				...
				if (player == 1){
					...
					if (this.displayMode == DisplayMode.CharacterPortrait){
						...
						CharacterInfo characterInfo = UFE.config.characters[characterIndex];
						...
						AnimationClip clip = 
							...
							characterInfo.moves[0].basicMoves.idle.clip1;
				...
				/*if (this.gameObjectPlayer2 != null && this.displayMode == DisplayMode.CharacterGameObject){
					CharacterInfo p2CharacterInfo = UFE.config.characters[this.p2HoverIndex];
