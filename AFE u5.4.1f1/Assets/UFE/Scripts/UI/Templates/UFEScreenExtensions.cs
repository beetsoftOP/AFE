		...
		if (slider != null){
			if (horizontalAxisDown && slider.direction == Slider.Direction.LeftToRight){
				if (slider.wholeNumbers){
					slider.value += Mathf.Sign(horizontalAxis);
				}else{
					slider.normalizedValue += Mathf.Sign(horizontalAxis)*UFEScreenExtensions.NormalizedSliderSpeed;
				}
				...
				if (slider.wholeNumbers){
					slider.value += Mathf.Sign(verticalAxis);
				}else{
					slider.normalizedValue += Mathf.Sign(verticalAxis) * UFEScreenExtensions.NormalizedSliderSpeed;
				}
				...
				screen.MoveCursor(new Vector3(horizontalAxis, verticalAxis), sound);
			}
		}else if (axisDown){
			screen.MoveCursor(new Vector3(horizontalAxis, verticalAxis), sound);
		}
