...
public class LegacyControl : MonoBehaviour {
    ...
    void Start() {
        ...
        if (currentAnimationData == null) currentAnimationData = animations[0];
    ...
    public void DoFixedUpdate() {
        ...
        currentAnimationData.animState.time += (Time.fixedDeltaTime * globalSpeed);
        animator.Sample();
    }
    ...
    public bool IsPlaying(string clipName) {
        if (currentAnimationData == GetAnimationData(clipName) 
            && currentAnimationData.wrapMode == WrapMode.ClampForever) return true;
